...
from ClergyRobotClasses.ClergyRobot import ClergyRobot
from Collection.eSkills import eSkills
from Humans.HumanFactory import HumanFactory
...
class Gameloop:
    def __init__(self):
        ...
        for i in range(0, 100):
            ...
            task = Task(randomNode.pos, self.taskListEventCallback, 200, 1, eSkills.CONVINCING)
        ...
        self.AICreatures = [ClergyRobot(i, random() * 8 * self.tileSize.y, random() * 8 * self.tileSize.y,
                                        self.window.tileLoader, self.tileSize, self.taskList) for i in
                            range(self.nmbrOfCreatures)]
    ...
    def preach(self):
        self.timedEventHandler.addTimedEvent(10000, self.peopleStartComingForService)

    def peopleStartComingForService(self):
        task = Task(Vector2(11 * self.tileSize.x, 5 * self.tileSize.y), self.preach, 250, 4, eSkills.CONVINCING,
                    "Preach")
        ...
        for i in range(randomAmountOfHumans):
            ...
            humanWaitTask = WaitTask(chair, None, "Listen to preacher", task, eSkills.STUBBORNNESS)
            self.humans.append(self.humanSpawner.initCreatureAtPos(entranceLists[int(random() * 2)], humanWaitTask))
